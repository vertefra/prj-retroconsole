...
| Fan + control surface | **`Body_FanMount`** (fan footprint / mount); **`Body_ControlPanel`** (button & stick layout on a face) | [Module 4](04_cooling_and_controls/README.md) |
| Outer case | **`Body_Shell`**: hollow **shell** with walls and **port cutouts** | [Module 5](05_enclosure_design/README.md) |
...
3. **Shell ↔ fan:** **`Body_FanMount`** (Module 4) is either **merged into `Body_Shell`** or stays a **sub-part** fixed to the shell or Pi area so airflow lines up with the SoC.
4. **Shell ↔ controls:** **`Body_ControlPanel`** carries **stick + buttons** on a **face** of the assembly (top, sloped deck, etc.—defined as you progress); **hole diameters** follow **component_specs.md** and **Module 4**.
